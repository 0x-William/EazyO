...
+ (id)parallaxHeaderViewWithImage:(UIImage *)image forSize:(CGSize)headerSize withPadding:(int)pad;
{
    ...
    headerView.padding = pad;
    [headerView initialSetupForDefaultHeader];
    
    headerView.imageScrollView.contentInset = UIEdgeInsetsMake(pad,pad,pad,pad);
...
{
    CGRect frame = CGRectMake(0, 0, self.bounds.size.width-_padding*2, self.bounds.size.height-_padding);
